...
def enhance_masks(data_folder, category_txt, color_list_path, num_class=104, area_thr=0, ratio_thr=0.5, top_k=80,
    ...
    for img_folder in os.listdir(data_folder):
        ...
        if not os.path.exists(pred_mask_path):
            print(f"Warning: {pred_mask_path} does not exist, skipping.")
            continue

        pred_mask = cv2.imread(pred_mask_path)
        if pred_mask is None:
            print(f"Warning: {pred_mask_path} could not be read, skipping.")
            continue
        pred_mask = pred_mask[:, :, 2]  # Use the red channel as the mask
